...
botonCifrarVigenere.addEventListener("click", handleButton);
botonDescifrarVigenere.addEventListener("click", handleButton);
...
function vigenere(llave, mensaje){
    ...
    // Si la llave tiene una longitud mayor que el mensaje a cifrar, se corta 
    // la llave Ej. llave=hola, mensaje=oso, la nueva llave sería hol
    if(llave.length > mensaje.length){
        llave = llave.slice(0, mensaje.length)
    }

    // Si el mensaje tiene una longitud mayor a la llave, la llave se 
    // extenderá Ej. llave=hola, mensaje=cortina, la nueva llave sería holahol
